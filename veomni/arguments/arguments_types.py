...
class ModelArguments:
    ...
    lora_target_modules: str = field(
        ...
        metadata={"help": "Comma-separated module names to train with LoRA."},
    )
    lora_target_modules_support: str = field(
        default="q_proj,k_proj,v_proj,o_proj,gate_proj,up_proj,down_proj",
        metadata={"help": "Comma-separated allowlist for LoRA target modules."},
    )
    lora_rank: int = field(
        default=16,
    ...
    init_lora_weights: str = field(
        default="kaiming",
        metadata={"help": "LoRA initialization strategy. 'kaiming' maps to PEFT default True."},
    )
    pretrained_lora_path: Optional[str] = field(
        default=None,
        metadata={"help": "Optional path to pretrained LoRA weights."},
    )
...
class DataArguments:
    ...
    audio_his_drop_prob: float = field(
        default=0.0,
        metadata={"help": "Probability to drop a random continuous span inside audio history tokens."},
    )
    audio_his_drop_ratio_max: float = field(
        default=0.0,
        metadata={"help": "Maximum ratio for random continuous span deletion inside audio history tokens."},
    )
    audio_his_drop_skip_prefix_tokens: int = field(
        default=0,
        metadata={"help": "Number of initial tokens inside audio history to protect from random span deletion."},
    )
...
class TrainingArguments:
    ...
    partial_train_param_patterns: str = field(
        default="",
        metadata={
            "help": (
                "Comma-separated regex patterns for base-model parameters to keep trainable. "
                "In LoRA mode, matching base parameters are trained in addition to LoRA adapters."
            )
        },
    )
    partial_train_last_n_layers: int = field(
        default=0,
        metadata={
            "help": (
                "When greater than 0, keep the last N transformer layers trainable. "
                "In LoRA mode, these base parameters are trained in addition to LoRA adapters."
            )
        },
    )
    lr: float = field(
    ...
    load_model_only: bool = field(
        default=False,
        metadata={"help": "When resuming from a checkpoint, load model weights only and reset optimizer/scheduler/dataloader states."},
    )
    save_steps: int = field(
        default=0,
        metadata={"help": "Number of steps between two checkpoint saves."},
    )
    save_best_steps: int = field(
        default=0,
        metadata={
            "help": "Check training loss every N steps and save only when it improves over the previous best check."
        },
    )
    logging_steps: int = field(
        default=1,
        metadata={"help": "Number of training steps between two progress-bar refreshes on rank0."},
    )
    save_epochs: int = field(
        default=1,
        metadata={"help": "Number of epochs between two checkpoint saves."},
    )
    save_epoch_points: List[float] = field(
        default_factory=list,
        metadata={
            "help": "Absolute fractional epoch points to save checkpoints at, e.g. 0.5 1.0 1.5. "
            "These are checked in addition to save_steps/save_epochs."
        },
    )
    ...
    keep_last_checkpoints: int = field(
        default=0,
        metadata={
            "help": "If > 0, keep only the latest N step checkpoints under output_dir/checkpoints and delete older ones."
        },
    )
    keep_best_and_last_checkpoints: bool = field(
        default=False,
        metadata={
            "help": "If enabled, prune step checkpoints to the best training-loss checkpoint and the latest checkpoint."
        },
    )
    seed: int = field(
        default=42,
