...
SUPPORTED_DATA_FILE_EXTENSIONS = {"parquet", "jsonl", "json", "csv", "arrow"}
...
def _get_data_file_extension(path: str) -> str:
...
def get_data_files(train_path):
    ...
    for data_path in data_paths:
        if data_path.startswith("hdfs://"):
            ...
            for filename in listdir(data_path):
                if _get_data_file_extension(filename) not in SUPPORTED_DATA_FILE_EXTENSIONS:
                    continue
            ...
            data_files.extend(
                [
                    ...
                    if os.path.isfile(os.path.join(data_path, fn))
                    and _get_data_file_extension(fn) in SUPPORTED_DATA_FILE_EXTENSIONS
                ]
            )
    ...
    if not data_files:
        raise ValueError(
            f"No supported data files found in {train_path}. "
            f"Supported extensions: {sorted(SUPPORTED_DATA_FILE_EXTENSIONS)}."
        )

    file_extenstion = _get_data_file_extension(data_files[0])
    if file_extenstion not in SUPPORTED_DATA_FILE_EXTENSIONS:
