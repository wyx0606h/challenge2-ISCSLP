    ...
    **kwargs,
    ...
    if query.dtype == torch.float32:
        ...
        # Handle the case where the model is quantized
            ...
            # FlashAttention kernels only support fp16/bf16 inputs.
            # If upstream modules keep activations in fp32 (common with some norm paths),
            # force a supported dtype instead of passing fp32 through.
            if target_dtype == torch.float32:
                if query.is_cuda and torch.cuda.is_bf16_supported():
    ...
    # Keep downstream projection dtype-consistent with the original attention input.
    if attn_output.dtype != original_query_dtype:
        attn_output = attn_output.to(original_query_dtype)
